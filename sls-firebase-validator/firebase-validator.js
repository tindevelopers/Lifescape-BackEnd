...
exports.handler = (event, context, callback) => {
    try {
        ...
        var refreshToken = event.authorizationToken;
        ...
        //New Code To Get Refresh Token
        var options = {
          method: 'POST', 
          //headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, 
          data: { grant_type: "refresh_token", refresh_token: refreshToken},
          url: "https://securetoken.googleapis.com/v1/token?key=" + process.env.webapikey
        };
...
// build apiOptions for the AuthPolicy
        ...
        var queryExecute = function() {
                ...
                .then(function (response) {//If successful api
                    console.log("verifying token ==>" + response.data.id_token);
                    ...
                    .then(function(decodedToken) {
                        ...
                        // you can send a 401 Unauthorized response to the client by failing like so:
                        console.log('validation.error1: ' + error + JSON.stringify(event));
                        callback('Unauthorized');
                    });

                }).catch(function(error) {
                        // you can send a 401 Unauthorized response to the client by failing like so:
                        console.log('validation.error: ' + error + JSON.stringify(event));
